...
for t=1:T
    ...
    % PID control
    R = eta + Kp*Ep + Ki*Ei + Kd*Ed; 
    
    % limit saturation
    if R < -1 
        R = -1; 
        disp(sprintf('Direct load control saturation (t=%.0f, R=-1)',t));
    elseif R > 1 
        R = 1; 
        disp(sprintf('Direct load control saturation (t=%.0f, R=+1)',t));
    end
    ...
    [dNoff dNon] = dN(Noff, Non, 0, R, phi);
...
set(get(AX(2),'Ylabel'),'String','Direct load control signal (\eta)');
